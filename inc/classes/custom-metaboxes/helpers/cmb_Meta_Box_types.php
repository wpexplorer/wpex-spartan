...
class cmb_Meta_Box_types {
	...
	private static function repeat_table_close() {
		echo '</tbody></table><p class="add-row"><a data-selector="', cmb_Meta_Box::$field['id'] ,'_repeat" class="add-row-button button" href="#">'. __( 'Add Row', 'spartan' ) .'</a></p>';
	}
	...
	private static function repeat_cell( $input ) {
		return '<td>'. $input .'</td><td class="remove-row"><a class="button remove-row-button" href="#">'. __( 'Remove', 'spartan' ) .'</a></td>';
	}
	...
	public static function textarea_code( $field, $meta ) {
		echo '<pre><textarea name="', $field['id'], '" id="', $field['id'], '" cols="60" rows="3" class="cmb_textarea_code">', ! empty( $meta ) ? $meta : $field['default'], '</textarea></pre>', self::desc( true );
	}
	...
	public static function file_list( $field, $meta, $object_id ) {
		...
		echo '<input class="cmb_upload_button button cmb_upload_list" type="button" value="'. __( 'Add or Upload File', 'spartan' ) .'" />', self::desc( true );
			...
			foreach ( $meta as $id => $fullurl ) {
				if ( self::is_valid_img_ext( $fullurl ) ) {
					...
					'<li class="img_status">',
						...
						'<p><a href="#" class="cmb_remove_file_button">'. __( 'Remove Image', 'spartan' ) .'</a></p>
					...
					echo
					'<li>',
						__( 'File:', 'spartan' ), ' <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; (<a href="', $fullurl, '" target="_blank" rel="external">'. __( 'Download', 'spartan' ) .'</a> / <a href="#" class="cmb_remove_file_button">'. __( 'Remove', 'spartan' ) .'</a>)
						<input type="hidden" id="filelist-', $id ,'" name="', $field['id'] ,'[', $id ,']" value="', $fullurl ,'" />
					</li>';
				}
			}
	...
	public static function file( $field, $meta, $object_id, $object_type ) {
		...
		echo '<input class="cmb_upload_button button" type="button" value="'. __( 'Add or Upload File', 'spartan' ) .'" />';
		...
		'<div id="', $field['id'], '_status" class="cmb_media_status">';
			if ( ! empty( $meta ) ) {
				...
				if ( self::is_valid_img_ext( $meta ) ) {
					...
					echo '<p><a href="#" class="cmb_remove_file_button" rel="', $field['id'], '">'. __( 'Remove Image', 'spartan' ) .'</a></p>';
					echo '</div>';
				} else {
					...
					echo __( 'File:', 'spartan' ), ' <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; (<a href="', $meta, '" target="_blank" rel="external">'. __( 'Download', 'spartan' ) .'</a> / <a href="#" class="cmb_remove_file_button" rel="', $field['id'], '">'. __( 'Remove', 'spartan' ) .'</a>)';
				}
			}
		echo '</div>';
	}
